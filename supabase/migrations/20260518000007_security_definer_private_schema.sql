-- 20260518000007_security_definer_private_schema.sql
-- Defense-in-depth: move SECURITY DEFINER bodies into a `private` schema so
-- they aren't directly reachable via PostgREST (which only exposes `public`).
-- Keep thin SECURITY INVOKER wrappers in `public` that forward to the private
-- implementations, preserving the client-facing RPC names.
--
-- Within SECURITY DEFINER bodies, `auth.uid()` still reflects the original
-- caller because it reads `request.jwt.claims` (a per-request GUC), not the
-- function owner — so the friendship/identity checks inside continue to work.
